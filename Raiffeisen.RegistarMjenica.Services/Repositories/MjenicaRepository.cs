...
public class MjenicaRepository : GenericRepository<Mjenica, MjenicaSearchObject>, IMjenicaRepository
{
    ...
    public override IQueryable<Mjenica> AddFilter(IQueryable<Mjenica> query, MjenicaSearchObject search = null)
    {
        ...
        if (search.Id.HasValue) filteredQuery = filteredQuery.Where(x => x.Id == search.Id.Value).AsQueryable();

        if (!string.IsNullOrWhiteSpace(search.ClientMjenicaSerialNumber))
            filteredQuery = filteredQuery
                .Where(x => x.ClientMjenicaSerialNumber.Contains(search.ClientMjenicaSerialNumber)).AsQueryable();

        if (!string.IsNullOrWhiteSpace(search.GuarantorMjenicaSerialNumber))
            filteredQuery = filteredQuery
                .Where(x => x.GuarantorMjenicaSerialNumber.Contains(search.GuarantorMjenicaSerialNumber)).AsQueryable();
        ...
        if (search.ContractDateStart != null)
            filteredQuery = filteredQuery
                .Where(x => x.ContractDate.HasValue && x.ContractDate.Value.Date >= search.ContractDateStart)
                .AsQueryable();

        if (search.ContractDateEnd != null)
            filteredQuery = filteredQuery
                .Where(x => x.ContractDate.HasValue && x.ContractDate.Value.Date <= search.ContractDateEnd)
                .AsQueryable();

        if (search.CreatedDateStart != null)
            filteredQuery = filteredQuery
                .Where(x => x.CreatedDate.HasValue && x.CreatedDate.Value.Date >= search.CreatedDateStart)
                .AsQueryable();

        if (search.CreatedDateEnd != null)
            filteredQuery = filteredQuery
                .Where(x => x.CreatedDate.HasValue && x.CreatedDate.Value.Date <= search.CreatedDateEnd).AsQueryable();
